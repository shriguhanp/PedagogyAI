...
export default function KnowledgePage() {
  ...
  useEffect(() => {
    ...
    kbs.forEach((kb) => {
      ...
      const encodedKbName = encodeURIComponent(kb.name);
      const ws = new WebSocket(
        wsUrl(`/api/v1/knowledge/${encodedKbName}/progress/ws`),
      );
      ...
      ws.onerror = (event: Event) => {
        // The browser provides a generic Event for websocket errors; inspect connection state
        try {
          console.error(`[Progress WS] Error for ${kb.name}:`, {
          ...
          console.error(`[Progress WS] Error for ${kb.name}: (failed to stringify event)`, e);
        }

        // Mark an error state for this KB so the UI can react (and persist)
        try {
          setProgressMap((prev) => ({
            ...prev,
            [kb.name]: {
          ...
          // Swallow errors to avoid crashing the component
        }
      };

      ws.onclose = (ev: CloseEvent) => {
        console.log(
          `[Progress WS] Closed for KB: ${kb.name} (code=${ev?.code ?? "?"}, reason=${ev?.reason || ""})`,
        );

        // If the close was abnormal, mark error state
        if (ev && ev.code && ev.code !== 1000) {
          try {
            setProgressMap((prev) => ({
              ...
              [kb.name]: {
                ...(prev[kb.name] || {}),
                error: `WebSocket closed (code=${ev.code})`,
              },
            }));
          } catch (e) {
            // ignore
  ...
  const handleDelete = async (name: string) => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/v1/knowledge/${encodeURIComponent(name)}`), {
        method: "DELETE",
      });
  ...
  const handleDeleteFile = async (kbName: string, fileName: string) => {
    if (
      !confirm(
        t("Are you sure you want to delete {{file}} from {{kb}}?", {
          file: fileName,
          kb: kbName,
        }),
      )
    ...
    try {
      ...
      const res = await fetch(
        apiUrl(
          `/api/v1/knowledge/${encodedKbName}/files/${encodedFileName}`,
        ),
        {
          method: "DELETE",
        },
      ...
      showToast(`Deleted ${fileName} from ${kbName}`, "success");
      // Refresh KB list
      fetchKnowledgeBases();
    } catch (err) {
      console.error(err);
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      const encodedTargetKb = encodeURIComponent(targetKb);
      const res = await fetch(apiUrl(`/api/v1/knowledge/${encodedTargetKb}/upload`), {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="animate-fade-in h-screen overflow-y-auto p-6">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      {kb.statistics.rag_provider && (
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border ${kb.statistics.rag_provider === "raganything"
                  ...
                  {!kb.is_default && (
                    <button
                      onClick={async () => {
                        try {
                          const res = await fetch(
                            apiUrl(`/api/v1/knowledge/default/${encodeURIComponent(kb.name)}`),
                            {
                              method: "PUT",
                            },
                          );
                      ...
                      return (
                        <div className="mt-2 space-y-1">
                          <div className="flex gap-3 text-[10px] text-slate-400 dark:text-slate-500">
                            <span>{kb.statistics.rag.chunks} {t("chunks")}</span>
                            <span>•</span>
                            <span>{kb.statistics.rag.entities} {t("entities")}</span>
                          </div>
                          {kb.statistics.rag_provider && (
                ...
                {kb.statistics.raw_documents > 0 && (
                  <div className="mt-4 border-t border-slate-200 dark:border-slate-700 pt-4">
                    <div className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-2 flex items-center gap-1.5">
                      <FileText className="w-3 h-3" /> {t("Documents")} ({kb.statistics.raw_documents})
                    </div>
                    <div className="max-h-32 overflow-y-auto space-y-1">
                      {/* Note: Files list would be populated from KB metadata when available */}
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 italic">
                        {t("Click 'Upload Documents' to manage files")}
                      </p>
                    </div>
                  </div>
                )}
      ...
      {/* Toast Notification */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-[100] px-4 py-3 rounded-lg text-sm font-medium text-white shadow-lg animate-in fade-in slide-in-from-bottom-2 ${
          toast.type === "success" ? "bg-emerald-500 dark:bg-emerald-600" :
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95">
            ...
            <form onSubmit={handleUpload} className="space-y-4">
              ...
              <div
                className={`border-2 border-dashed rounded-xl transition-colors ${dragActive
                    ? "border-blue-500 bg-blue-50 dark:bg-blue-900/30"
                    : "border-slate-200 dark:border-slate-600 hover:border-blue-400 dark:hover:border-blue-500 bg-slate-50 dark:bg-slate-700/50"
                  }`}
                onDragEnter={handleDrag}
              ...
              >
                ...
                {uploadFiles.length > 0 && (
                  <div className="border-t border-slate-200 dark:border-slate-600 px-3 py-2 max-h-48 overflow-y-auto">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-medium text-slate-500 dark:text-slate-400">
                        {t("{{count}} file{{plural}} selected", {
                          count: uploadFiles.length,
                          plural: uploadFiles.length > 1 ? "s" : "",
                        })}
                      </span>
                      <button
                        type="button"
